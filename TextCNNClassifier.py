...
class TextCNNClassifier(object):
    ...
    def predict_accuracy(self,data,test=True):
        ...
        for batch in batches:
            ...
            batch_pred, batch_accuracy, batch_loss = sess.run([self.predictions,self.accuracy, self.loss], feed_dict=feed)
            accuracy_list.append(batch_accuracy)
            predictions.append(batch_pred)
            print('The trainning step is {0}'.format(iterations),'trainning_accuracy: {:.3f}'.format(accuracy_list[-1]))
